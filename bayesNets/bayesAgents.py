...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    housePositions = [topLeftPos, topRightPos, bottomLeftPos, bottomRightPos]

    for i, housePos in enumerate(housePositions):
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                ghost = assignment[GHOST_HOUSE_VAR] == HOUSE_VALS[i]
                food = assignment[FOOD_HOUSE_VAR] == HOUSE_VALS[i]
                red = assignment[obsVar] == RED_OBS_VAL
                blue = assignment[obsVar] == BLUE_OBS_VAL
                none = assignment[obsVar] == NO_OBS_VAL
                if not ghost and not food:
                    if none:
                        prob = 1.0
                    else:
                        prob = 0.0
                elif ghost and not food:
                    if red:
                        prob = PROB_GHOST_RED
                    elif blue:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for assignmentDict in factor.getAllPossibleAssignmentDicts():
        ...
        if prob > bestProb:
            bestProb = prob
            bestAssignmentDict = assignmentDict
    return bestAssignmentDict
