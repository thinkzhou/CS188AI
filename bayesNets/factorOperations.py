...
def joinFactors(factors):
    ...
    retFactor = deepcopy(factors[0])
    for factor in factors[1:]:
        retFactor = joinTwoFactors(retFactor, factor)
    return retFactor
...
def normalize(factor):
    ...
    conditionedVariables = [x for x in factor.conditionedVariables()]
    for x in factor.unconditionedVariables():
        if len(factor.variableDomainsDict().get(x))>1:
           unconditionedVariables.append(x)
        else:
            conditionedVariables.append(x)
    retFactor = Factor(unconditionedVariables, conditionedVariables, factor.variableDomainsDict())
    sum = 0.0
    for assignmentDict in retFactor.getAllPossibleAssignmentDicts():
        sum += factor.getProbability(assignmentDict)

    for assignmentDict in retFactor.getAllPossibleAssignmentDicts():
        newProb = factor.getProbability(assignmentDict) / sum
        retFactor.setProbability(assignmentDict, newProb)
    return retFactor
